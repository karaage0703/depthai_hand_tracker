...
class HandTrackerRenderer:
    ...
    def draw_hand(self, hand):
        ...
        if self.show_xyz:
            ...
            cv2.rectangle(self.frame, (x0, y0), (x0+100, y0+40), (220, 220, 240), -1)
            ...
            cv2.putText(self.frame, f"Z:{hand.xyz[2]:3.0f} mm", (x0+10, y0+20), cv2.FONT_HERSHEY_PLAIN, 1, (0, 0, 255), 2)
    ...
    def draw(self, frame, hands):
        ...
        self.frame = np.zeros((image_h, image_w, 3), np.uint8)
        for hand in hands:
            self.draw_hand(hand)
        return self.frame

    def audio(self, hands):
        psonic.use_synth(psonic.PULSE)
        for hand in hands:
            try:
                self.freq = ((1 / hand.xyz[2]) * 3500)**2 + 20
            except ZeroDivisionError:
                print('zero division eroor')
            print(hand.gesture)
            if hand.gesture == 'OK':
                psonic.use_synth(psonic.PULSE)
            if hand.gesture == 'ONE':
            ...
            if hand.gesture == 'FOUR':
                psonic.use_synth(psonic.PULSE)
            if hand.gesture == 'FIVE':
                psonic.use_synth(psonic.PIANO)
            if hand.gesture == 'FIST':
                psonic.use_synth(psonic.CHIPLEAD)

        with psonic.Fx(psonic.REVERB):
            psonic.play(self.freq, release=0.4)
        print(self.freq)
        # sleep(0.01)
